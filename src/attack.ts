...
export async function main(ns: NS): Promise<void> {
    ...
    ns.print(`Found attackers: (${attackers.length}) [${attackers.map(a => a.hostname).reduce((acc, cur) => acc + cur + ', ', '')}]`);
    ...
    if (weakenTarget) {
        ...
        while (true) {
            secDecrease = weakenTarget.security.current - weakenTarget.security.min;
            weakenThreads = Math.ceil(secDecrease * 20.5);
            ns.print(`${weakenTarget.hostname} Sec: +${secDecrease}`);
            ns.print(`Weakening ${weakenTarget.hostname} by ${secDecrease} with ${weakenThreads} threads.`);
            break;
        }
        ...
        if (weakenThreads > 0)
            ns.print(`${weakenThreads} threads more needed.`);

        ns.print(`Should decrease by ${ns.weakenAnalyze(orgThreads - weakenThreads)} (+${secDecrease})`);
    }
